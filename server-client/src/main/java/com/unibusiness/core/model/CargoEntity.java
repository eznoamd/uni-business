...
public class CargoEntity {
    ...
    @ManyToMany(mappedBy = "cargos", fetch = FetchType.LAZY) private Set<UsuarioEntity> usuarios = new HashSet<>();
    @ManyToMany(fetch = FetchType.LAZY)
    @JoinTable(name = "cargo_permissao", joinColumns = @JoinColumn(name = "cargo_id"), inverseJoinColumns = @JoinColumn(name = "permissao_id"))
    private Set<PermissaoEntity> permissoes = new HashSet<>();
    public CargoEntity() {}
    public CargoEntity(String nome) { this.nome = nome; }
