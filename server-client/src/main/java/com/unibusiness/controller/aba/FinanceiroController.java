...
public class FinanceiroController {
    ...
    private void carregarCaixa() {
        ...
        Task<Dto.Caixa> task = new Task<>() {
            @Override protected Dto.Caixa call() {
                Dto.Caixa atual = caixaService.getCaixaAtual();
                // Sem caixa aberto: busca o último fechado só para exibição
                // (mostrar o saldo final em vez de "—").
                return atual != null ? atual : caixaService.getUltimoFechado();
            }
        };
    ...
    private void atualizarUIcaixa() {
        Platform.runLater(() -> {
            ...
            } else if (caixaAtual != null) {
                // Não há caixa aberto, mas existe um fechado anteriormente —
                // mostra o saldo final dele em vez de "—".
                lblSaldoAtual.setText("R$ " + String.format("%.2f", caixaAtual.getSaldoAtual()));
                lblSaldoInicial.setText("R$ " + String.format("%.2f", caixaAtual.getSaldoInicial()));
                lblAbertura.setText(caixaAtual.getFechamentoEm() != null
                    ? "Fechado em " + caixaAtual.getFechamentoEm().substring(0, 16) : "—");
